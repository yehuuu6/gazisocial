...
class HunterActionHandler implements ActionHandlerInterface
{
    ...
    public function handle(GameAction $action): void
    {
        ...
        if ($playerToKill->role->enum === PlayerRole::WITCH) {
            ...
            $this->sendMessageToPlayer($playerToKill, "Bir avcı seni öldürmeyi denedi, ama sen onun zihnine sahte bir görüntü yerleştirdin ve kurtuldun.", ChatMessageType::WARNING);
            return;
        }
        ...
        $this->sendMessageToPlayer($playerToKill, 'Bir avcı tarafından vuruldun!', ChatMessageType::WARNING);
        // Check if the target was a town role
        if (in_array($playerToKill->role->enum, PlayerRole::getTownRoles())) {
