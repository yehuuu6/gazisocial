<x-modals.modal-wrapper x-show="shareModal">
    <x-modals.modal-inner-wrapper x-show="shareModal" x-on:click.away="shareModal = false">
        <div class="bg-white">
            <div class="px-4 py-4">
            ...
            <div class="m-4 flex items-center gap-3">
                <a target="_blank" href="https://api.whatsapp.com/send?text={{ urlencode($url) }}"
                    class="overflow-hidden rounded-full p-3 grid place-items-center shadow-lg bg-green-500 duration-300 transition-all hover:scale-105">
                    <x-icons.social.whatsapp size="30" color="white" />
                </a>
                <a target="_blank" href="https://x.com/intent/post?url={{ urlencode($url) }}"
                    class="overflow-hidden rounded-full p-3 grid place-items-center shadow-lg bg-black duration-300 transition-all hover:scale-105">
                    <x-icons.social.x size="30" color="#dadada" />
                </a>
                <a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($url) }}"
                    class="overflow-hidden rounded-full p-3 grid place-items-center shadow-lg bg-blue-600 duration-300 transition-all hover:scale-105">
                    <x-icons.social.facebook size="30" color="white" />
                </a>
            </div>
            <div class="m-4 flex items-center gap-2 bg-gray-100 rounded-md border border-gray-300">
                <input type="text" readonly id="post-url"
                    class="text-gray-500 bg-transparent font-medium w-full px-6 py-4 outline-none"
                    value="{{ $url }}" />
                <div x-data="{
                    copyText: '{{ $url }}',
            ...
            <div class="flex items-center justify-end gap-2 bg-gray-50 p-4">
                <button type="button" x-on:click="shareModal = false"
