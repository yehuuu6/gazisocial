        ...
        <div class="flex-grow">
            <div class="flex flex-col gap-5 py-4">
                <div class="flex flex-col gap-2 px-4">
                    <label for="title" class="block text-lg font-medium text-gray-700">Başlık</label>
                    <input wire:model="title" type="text" id="title" name="title" maxlength="100" minlength="6"
                        required class="block w-full rounded-md border border-gray-200 px-3 py-2 outline-none" />
                </div>
                <div x-data="{ selectedTags: $wire.selectedTags }" class="flex flex-col gap-2 px-4">
                    <h4 class="block cursor-default font-medium text-lg  text-gray-700">Etiketler</h4>
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach ($this->tags as $tag)
                            <button
                                ...
                                type="button"
                                :class="selectedTags.includes({{ $tag->id }}) ? 'bg-{{ $tag->color }}-500 text-white' :
                                    'bg-gray-50 hover:bg-gray-100 text-gray-700'"
                                class="rounded-full px-4 py-1.5 shadow-sm focus:outline-none text-sm active:scale-90 transition-transform duration-100">
                                {{ $tag->name }}
                            </button>
                        @endforeach
                    </div>
                ...
                <div class="flex flex-col gap-2 px-4">
                    <h4 class="block font-medium text-lg  text-gray-700">İçerik</h4>
                    <x-editor spellcheck="false"></x-editor>
                </div>
                <div class="flex flex-col gap-2 px-4">
                    <h3 class="block cursor-default text-lg  font-medium text-gray-700">Anketler</h3>
