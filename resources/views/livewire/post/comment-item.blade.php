<div>
    <div x-data="{
        ...
        showReplies = true;
    })" x-on:click.away="replyForm = false;">
        <div :class="{ 'mt-2.5': {{ $comment->depth }} > 0 }">
            <div class="flex items-center gap-2 w-fit pr-3" x-ref="comment{{ $comment->id }}">
                ...
                <x-comment.comment-details :$comment :$post />
            </div>
            <div class="relative">
                @if ($comment->replies_count > 0)
                    <div x-on:click="$refs.avatar.scrollIntoView({ behavior: 'smooth', block: 'center'}); $refs.comment{{ $comment->id }}.classList.add('highlight'); setTimeout(() => { $refs.comment{{ $comment->id }}.classList.remove('highlight'); }, 2000);"
                        class="absolute top-0 bottom-0 left-4 w-4 mb-5 z-[1] group">
                        <div
                            class="w-full h-full border-l border-gray-200 group-hover:border-l-2 group-hover:border-gray-500 group-hover:cursor-pointer">
                        </div>
                    </div>
                @endif
                <div class="ml-10">
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="relative flex items-center gap-0.5 mt-2 flex-wrap">
                            ...
                            <x-comment.comment-button x-on:click="open = !open;" x-ref="moreButton">
